...
function ModoConsulta({ onVoltar }) {
  ...
  async function buscar(e) {
    ...
    try {
      const data = await consultarProtocolo(p);
      setResultado(data);
  ...
  return (
    <div>
      ...
      <div style={{ fontSize:13, color:C.gray600, marginBottom:20 }}>
        Digite o número de protocolo recebido no WhatsApp.
      </div>
      <form onSubmit={buscar} style={{ display:"flex", gap:8, marginBottom:16 }}>
        <input
          ...
          style={{ flex:1, border:"2px solid " + C.gray200, borderRadius:10,
      ...
      {erro && (
        <div style={{ color:C.red, fontSize:13, marginBottom:12 }}>⚠️ {erro}</div>
      )}
      {resultado && <CardInfo ag={resultado} />}
      <button onClick={onVoltar}
        style={{ background:"transparent", color:C.gray600, border:"none",
          padding:0, fontFamily:"'Nunito',sans-serif", fontSize:13,
...
function ModoReagendar({ token, onVoltar }) {
  ...
  useEffect(() => {
    if (!dataSel) return;
    buscarSlotsOcupados(dataSel).then(setSlotsOcupados);
  }, [dataSel]);
...
export default function ConsultaAgendamento({ modo, token, onVoltar }) {
  return (
    <div style={{ fontFamily:"'Nunito',sans-serif", minHeight:"100vh",
      ...
      <div style={{ width:"100%", maxWidth:480 }}>
        <div style={{ background:C.white, borderRadius:20, padding:"28px 24px",
          ...
          {modo === "cancelar" && <ModoCancelar token={token} onVoltar={onVoltar} />}
          {modo === "reagendar" && <ModoReagendar token={token} onVoltar={onVoltar} />}
          {modo === "consulta" && <ModoConsulta onVoltar={onVoltar} />}
        </div>
      </div>
    </div>
  );
